...
export default {
  ...
  created() {
    ...
    this.newRefresh = this.debounce(() => {
      this.$refs.scroll.refresh();
    });
  },
  methods: {
    debounce(func, delay) {
      //防抖 fnc是要执行的函数 把他传过来 delay到底等多久
      let timer = null;
    ...
    titleClick(index) {
      this.$refs.scroll.scroll.scrollTo(0, -this.themeTopYs[index], 100); //根据数组 下标不同点击按钮跳转到对应的位置
    },
